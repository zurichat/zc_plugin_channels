import { Box, Flex, Text, Link, Input, HStack } from '@chakra-ui/react';
import { IoChevronDownOutline } from 'react-icons/io5';
import { GrEmoji } from 'react-icons/gr';

const DisabledInput = () => {
  return (
    <>
      <Box
        border="1px"
        borderColor="#EBEBEB"
        padding="0.5em"
        borderRadius="5px"
      >
        <Input
          border="none"
          outline="none"
          marginBottom="1em"
          _focus={{ outline: 'none' }}
        />
        <Flex justify="space-between" px="1em" paddingBottom="0.5em">
          {/* LEFT */}
          <HStack spacing="11px">
            <svg
              width="12"
              height="14"
              viewBox="0 0 12 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M6.77683 0.407736C6.93311 0.475575 7.02596 0.638322 7.00483 0.807374L6.43472 5.36819H11.6162C11.7651 5.36819 11.9006 5.45433 11.9637 5.58918C12.0269 5.72403 12.0064 5.88325 11.911 5.99765L5.67079 13.4859C5.56172 13.6168 5.37943 13.6601 5.22315 13.5923C5.06687 13.5244 4.97403 13.3617 4.99516 13.1926L5.56526 8.63183H0.383779C0.234868 8.63183 0.0993986 8.54569 0.036237 8.41084C-0.0269245 8.27598 -0.00637525 8.11677 0.0889554 8.00237L6.32919 0.514086C6.43826 0.383206 6.62055 0.339896 6.77683 0.407736ZM1.20315 7.86428H5.99999C6.11007 7.86428 6.21485 7.91155 6.2877 7.99407C6.36055 8.0766 6.39446 8.18643 6.3808 8.29566L5.91914 11.989L10.7968 6.13574H5.99999C5.88991 6.13574 5.78513 6.08847 5.71228 6.00594C5.63943 5.92342 5.60553 5.81359 5.61918 5.70436L6.08084 2.01105L1.20315 7.86428Z"
                fill="#8B8B8B"
              />
            </svg>

            <svg
              width="2"
              height="18"
              viewBox="0 0 2 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M1 1V17" stroke="#EBEBEB" stroke-linecap="round" />
            </svg>

            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M2.37988 2.33317C2.37988 2.03671 2.62021 1.79639 2.91666 1.79639H7.58333C9.16845 1.79639 10.4534 3.08138 10.4534 4.6665C10.4534 6.25162 9.16845 7.53661 7.58333 7.53661H2.91666C2.62021 7.53661 2.37988 7.29629 2.37988 6.99983V2.33317ZM3.45344 2.86995V6.46305H7.58333C8.57554 6.46305 9.37988 5.65871 9.37988 4.6665C9.37988 3.67429 8.57554 2.86995 7.58333 2.86995H3.45344Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M2.37988 7.00016C2.37988 6.7037 2.62021 6.46338 2.91666 6.46338H8.16666C9.75178 6.46338 11.0368 7.74837 11.0368 9.33349C11.0368 10.9186 9.75178 12.2036 8.16666 12.2036H2.91666C2.62021 12.2036 2.37988 11.9633 2.37988 11.6668V7.00016ZM3.45344 7.53694V11.13H8.16666C9.15887 11.13 9.96322 10.3257 9.96322 9.33349C9.96322 8.34128 9.15887 7.53694 8.16666 7.53694H3.45344Z"
                fill="#8B8B8B"
              />
            </svg>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M5.29639 2.33317C5.29639 2.03671 5.53671 1.79639 5.83317 1.79639H11.0832C11.3796 1.79639 11.6199 2.03671 11.6199 2.33317C11.6199 2.62962 11.3796 2.86995 11.0832 2.86995H5.83317C5.53671 2.86995 5.29639 2.62962 5.29639 2.33317Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M2.37988 11.6667C2.37988 11.3702 2.62021 11.1299 2.91666 11.1299H8.16666C8.46312 11.1299 8.70344 11.3702 8.70344 11.6667C8.70344 11.9631 8.46312 12.2034 8.16666 12.2034H2.91666C2.62021 12.2034 2.37988 11.9631 2.37988 11.6667Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M8.93829 1.83071C9.21587 1.9348 9.35651 2.24421 9.25242 2.52179L5.75242 11.8551C5.64833 12.1327 5.33892 12.2733 5.06134 12.1692C4.78376 12.0652 4.64312 11.7557 4.74721 11.4782L8.24721 2.14484C8.35131 1.86726 8.66071 1.72662 8.93829 1.83071Z"
                fill="#8B8B8B"
              />
            </svg>

            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M8.44913 2.12448C9.96586 0.60639 12.3768 0.6281 13.8678 2.17327C15.3589 3.71844 15.3798 6.21688 13.9149 7.78867L13.9077 7.79645L11.9489 9.82621C11.9489 9.82623 11.9489 9.82618 11.9489 9.82621C11.156 10.6481 10.0576 11.0728 8.93895 10.9898C7.82026 10.9067 6.79153 10.3243 6.11974 9.39358C5.92098 9.11822 5.97526 8.72802 6.24098 8.52205C6.50669 8.31608 6.88322 8.37233 7.08198 8.64769C7.54493 9.28907 8.25386 9.69045 9.02479 9.74767C9.79571 9.80489 10.5526 9.51229 11.099 8.94583L13.054 6.91986C14.0601 5.83647 14.0445 4.11742 13.0181 3.05381C11.9916 1.98999 10.3323 1.97405 9.2869 3.01721L8.16709 4.17092C7.93177 4.41336 7.55136 4.41221 7.3174 4.16835C7.08345 3.92449 7.08456 3.53026 7.31988 3.28782L8.44913 2.12448Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M4.0512 6.17371C4.8441 5.35177 5.9424 4.92722 7.06105 5.01025C8.17974 5.09327 9.20847 5.67572 9.88026 6.60642C10.079 6.88178 10.0247 7.27197 9.75902 7.47794C9.49331 7.68391 9.11678 7.62766 8.91802 7.3523C8.45507 6.71093 7.74614 6.30954 6.97521 6.25233C6.20429 6.19511 5.44739 6.4877 4.90097 7.05417L2.94597 9.08012C1.93993 10.1635 1.9555 11.8826 2.98186 12.9462C4.00822 14.0098 5.66706 14.0259 6.7125 12.9834L7.82514 11.8303C8.05977 11.5872 8.44019 11.5872 8.67483 11.8303C8.90946 12.0735 8.90946 12.4677 8.67483 12.7109L7.55093 13.8756C6.0342 15.3936 3.62322 15.3719 2.13217 13.8267C0.641128 12.2815 0.620178 9.7831 2.08509 8.21132L2.09234 8.20354L4.0512 6.17371C4.05117 6.17374 4.05122 6.17369 4.0512 6.17371Z"
                fill="#8B8B8B"
              />
            </svg>

            <svg
              width="16"
              height="15"
              viewBox="0 0 16 15"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M4.55713 3.75023C4.55713 3.48016 4.77606 3.26123 5.04613 3.26123H13.2459C13.516 3.26123 13.7349 3.48016 13.7349 3.75023C13.7349 4.0203 13.516 4.23924 13.2459 4.23924H5.04613C4.77606 4.23924 4.55713 4.0203 4.55713 3.75023Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M4.55713 7.50023C4.55713 7.23016 4.77606 7.01123 5.04613 7.01123H13.2459C13.516 7.01123 13.7349 7.23016 13.7349 7.50023C13.7349 7.7703 13.516 7.98924 13.2459 7.98924H5.04613C4.77606 7.98924 4.55713 7.7703 4.55713 7.50023Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M4.55713 11.2502C4.55713 10.9802 4.77606 10.7612 5.04613 10.7612H13.2459C13.516 10.7612 13.7349 10.9802 13.7349 11.2502C13.7349 11.5203 13.516 11.7392 13.2459 11.7392H5.04613C4.77606 11.7392 4.55713 11.5203 4.55713 11.2502Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.01221 3.75023C3.01221 3.48016 2.79327 3.26123 2.5232 3.26123H1.89245C1.62238 3.26123 1.40345 3.48016 1.40345 3.75023C1.40345 4.0203 1.62238 4.23924 1.89245 4.23924H2.5232C2.79327 4.23924 3.01221 4.0203 3.01221 3.75023Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.01221 7.50023C3.01221 7.23016 2.79327 7.01123 2.5232 7.01123H1.89245C1.62238 7.01123 1.40345 7.23016 1.40345 7.50023C1.40345 7.7703 1.62238 7.98924 1.89245 7.98924H2.5232C2.79327 7.98924 3.01221 7.7703 3.01221 7.50023Z"
                fill="#8B8B8B"
              />
              <path
                d="M2.52295 11.25H1.8922"
                stroke="#8B8B8B"
                stroke-width="0.978006"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
          </HStack>

          {/* RIGHT */}
          <HStack spacing="11px">
            {/* @ icon */}
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.7736 8.07369C10.7736 9.61938 9.53198 10.8724 8.00036 10.8724C6.46873 10.8724 5.22711 9.61938 5.22711 8.07369C5.22711 6.528 6.46873 5.27497 8.00036 5.27497C9.53198 5.27497 10.7736 6.528 10.7736 8.07369ZM10.7736 8.07369L10.7737 8.77338C10.7737 9.93264 11.7049 10.8724 12.8536 10.8724C14.0023 10.8724 14.9336 9.93264 14.9336 8.77338V8.0737C14.9334 4.82222 12.7137 1.99937 9.57608 1.26043C6.4385 0.521487 3.20998 2.06121 1.78367 4.97672C0.357374 7.89224 1.10922 11.4151 3.59833 13.4796C6.08745 15.5441 9.6578 15.606 12.2158 13.6292"
                stroke="#8B8B8B"
                stroke-width="0.941892"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
            {/* attachment icon */}
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M14.2919 7.77498L8.16523 13.9016C6.60227 15.4646 4.0682 15.4646 2.50523 13.9016C0.942266 12.3387 0.942266 9.80461 2.50523 8.24165L8.6319 2.11498C9.67388 1.073 11.3633 1.073 12.4052 2.11498C13.4472 3.15696 13.4472 4.84633 12.4052 5.88831L6.2719 12.015C5.75091 12.536 4.90622 12.536 4.38523 12.015C3.86424 11.494 3.86424 10.6493 4.38523 10.1283L10.0452 4.47498"
                stroke="#8B8B8B"
                stroke-width="0.941892"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
            {/* emoji icon */}
            <GrEmoji fontSize="1.3em" color="#8b8b8b" />
            {/* send message icon */}
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M13.5 7.99023L2 2.99023H1.5L4 7.99023L1.5 12.9902L13.5 7.99023Z"
                fill="#8B8B8B"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M14.6392 8.06274C14.6392 8.30644 14.5054 8.52416 14.3044 8.60773L1.45674 13.9482C1.25549 14.0318 1.02978 13.9632 0.891942 13.7765C0.754109 13.5897 0.733604 13.3247 0.840633 13.1134L3.39861 8.06274L0.840633 3.01211C0.733604 2.80079 0.754109 2.5358 0.891942 2.34904C1.02978 2.16227 1.25549 2.09364 1.45674 2.1773L14.3044 7.51776C14.5054 7.60133 14.6392 7.81905 14.6392 8.06274ZM2.42325 3.81428L4.42816 7.77291C4.51901 7.95229 4.51901 8.1732 4.42816 8.35258L2.42325 12.3112L12.6438 8.06274L2.42325 3.81428Z"
                fill="#8B8B8B"
              />
            </svg>
            {/* stroke */}
            <svg
              width="2"
              height="22"
              viewBox="0 0 2 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M1 1V17" stroke="#8B8B8B" stroke-linecap="round" />
            </svg>
            <IoChevronDownOutline color="#8b8b8b" />
          </HStack>
        </Flex>
      </Box>
      <br />
      <br />

      {/* disabled */}
      <Box border="1px" borderColor="#EBEBEB" borderRadius="5px" padding="2em">
        <Flex alignItems="center" justifyContent="center">
          <HStack
            spacing="11px"
            display={{ lg: 'inline-block', sm: 'none' }}
            position="fixed"
            left="5em"
          >
            <svg
              width="12"
              height="14"
              viewBox="0 0 12 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M6.77683 0.407736C6.93311 0.475575 7.02596 0.638322 7.00483 0.807374L6.43472 5.36819H11.6162C11.7651 5.36819 11.9006 5.45433 11.9637 5.58918C12.0269 5.72403 12.0064 5.88325 11.911 5.99765L5.67079 13.4859C5.56172 13.6168 5.37943 13.6601 5.22315 13.5923C5.06687 13.5244 4.97403 13.3617 4.99516 13.1926L5.56526 8.63183H0.383779C0.234868 8.63183 0.0993986 8.54569 0.036237 8.41084C-0.0269245 8.27598 -0.00637525 8.11677 0.0889554 8.00237L6.32919 0.514086C6.43826 0.383206 6.62055 0.339896 6.77683 0.407736ZM1.20315 7.86428H5.99999C6.11007 7.86428 6.21485 7.91155 6.2877 7.99407C6.36055 8.0766 6.39446 8.18643 6.3808 8.29566L5.91914 11.989L10.7968 6.13574H5.99999C5.88991 6.13574 5.78513 6.08847 5.71228 6.00594C5.63943 5.92342 5.60553 5.81359 5.61918 5.70436L6.08084 2.01105L1.20315 7.86428Z"
                fill="#8B8B8B"
              />
            </svg>
          </HStack>
          <Text color="#BEBEBE" textAlign="center">
            Only certain people can post in this channel.
            <Link
              color="#1264A3"
              href="#"
              textDecoration="underline"
              marginLeft="1ch"
            >
              Learn more
            </Link>
          </Text>
        </Flex>
      </Box>
    </>
  );
};

export default DisabledInput;

...
const MessageCard = ({
  ...
  const { workspaceUsersObject, users: authUser } = useSelector(
    state => state.appReducer
  )
  const user = workspaceUsersObject[userId] || {
    display_name: userId,
    image_url: ""
  }

  const pinMessage = () => {
    _pinMessage(authUser.org_id, channelId, userId, _id)
  }
        ...
        <Flex flexDir="column">
          ...
          <Box m="0px">
            ...
            >
              {content}{" "}
              {edited && (
                <Text fontSize="8px" display="contents">
                  (edited)
                </Text>
              )}
            </Text>
          ...
          {replies !== 0 && (
            <HStack spacing="5px" mt="5px">
              {threadReply
                .slice(0, Math.min(4, threadReply.length))
                .map((reply, index) => (
                  <Avatar
              ...
              <HStack spacing="5px" alignItems="baseline">
                <Link fontSize={["8px", "14px"]} color="#1264A3">
                  {threadReply.length} Replies
                </Link>
                <Text
                  fontSize={["8px", "12px"]}
                  color="#616061"

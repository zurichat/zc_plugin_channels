...
const MultimediaSharingModal = () => {
  ...
  const fileUploadHandler = async () => {
    ...
    axios
      .post(
        "https://channels.zuri.chat/api/v1/1/channels/614fd30bcf2c0f1ad758538e/messages/",
        fd
      )
      .then(
        await axios
          .get(
            "https://channels.zuri.chat/api/v1/1/channels/614fd30bcf2c0f1ad758538e/media/?format=json",
            fd
          )
          .then(res => {
    ...
    <Flex direction="column">
      <Stack direction="column" p={4} pb={0}>
        ...
        <Text pt={3} fontSize="small">
          <Link fontWeight="semibold" _focus="none" color="green.300" href="#">
            View all your files
          </Link>
        </Text>
      </Stack>
      ...
      <Stack direction="column" p={4} pt={0}>
        <Box pt={4} pb={2}>
          <Heading
            as="h3"
            size="lg"
...
const File = props => {
  const { snapshotUrl, fileName, fileAuthor, fileUploadDate, fileUploadTime } =
    props
  return (
    <HStack spacing={4}>
      <Image
